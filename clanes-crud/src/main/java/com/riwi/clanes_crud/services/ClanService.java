...
public class ClanService implements IClanService {
    ...
    public Clan create(ClanReq req) {
        Cohort cohort=this.cohortRepository.findById(req.getCohortId()).orElseThrow(()->new RuntimeException("cohort not found"));
    ...
    public Clan update(Long id, ClanUpdateReq req) {
        ...
        if (req.getCohortId() != clan.getCohort().getId()) {
            Cohort cohort = this.cohortRepository.findById(id).orElseThrow(()->new RuntimeException("cohort not found"));
